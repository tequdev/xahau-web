...
| `FinishAfter`    | Number                                                                                               | UInt32            | *(Optional)* The time, in [seconds since the Ripple Epoch][], when the escrowed Amount can be released to the recipient. This value is immutable; the funds cannot move until this time is reached.                                    |
